...
\pagebreak
\section{Levo in desno-linearne gramatike}\label{sec:LLGDLG}
Posebnost linearnih gramatik je v tem, da imajo na desni strani produkcij največ en vmesni simbol, ampak ta model je že močnejši od regularnih jezikov (glej \ref{sec:LG}), če pa se omejimo le na tiste produkcije, ki imajo ta edini vmesni simbol vedno na skrajno levi strani, ali pa vedno na skrajni desni strani niza, dobimo model, ki opisuje regularne jezike.
%
\Def{Regularna gramatika je definirana kot četvorček $G=\langle N,T,P,S \rangle$, kjer je:
\begin{items}
...
\section{Prevedbe med modeli regularnih jezikov}%?lame naslov, fixme
...
\pagebreak%delete this, if it gets weird :)
\subsection{Regularni izraz $\rightarrow$ Nedeterministični končni avtomat z $\varepsilon$-prehodi}\label{RI-eNKA}
%konstanta
...
%?kul bi blo obkrožit kaj so tu osnovni izrazi
Sestavljeni izrazi:
\begin{itemize}
\item $r = a + b$\\
\begin{tikzpicture}[>=latex',/tikz/initial text="", scale=\RIeNKAscale, every state/.style={scale=\RIeNKAscale}]]
...
\subsection{Desno-linearna gramatika $\rightarrow$ Nedeterministični končni avtomat z $\varepsilon$-prehodi}\label{DLG-eNKA}
...
\begin{tikzpicture}[>=latex',/tikz/initial text=""]
	...
	\node (bA)  at (80bp,-40bp)  [state]           {$bA$};
	\node (A)   at (120bp,0bp)   [state]           {$A$};
	\node (aa)  at (160bp,0bp)   [state]           {$aa$};
	\node (a)   at (200bp,0bp)   [state]           {$a$};
	\node (e)   at (240bp,0bp)   [state,accepting] {$\varepsilon$};
...
\section{Linearne gramatike}\label{sec:LG}

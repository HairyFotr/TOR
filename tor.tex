...
\Def{Totalne naravne funkcije $f:N^k\leftarrow N$, ki jih je moč konstruirati iz treh začetnih funkcij, s končno mnogo uporabami treh pravil sestavljanja imenujemo \textbf{rekurzivne funkcije.}
\br
...
\subsect{$\lambda$-račun (Alonso Church)}%1932,1933 Alonso Church
...
\item Uporablja posebno vrsto znakov
\end{items}
\fixme - slika
...
Algoritem je zaporedje korakov, ki postopno preoblikuje začetno besedo v končno. V vsakem koraku se trenutna beseda preoblikuje s najbolj levo možno produkcijo.
...
Church-Turingova teza: \Quote{algoritem} $\Longleftrightarrow$ algoritem po Turingu ali po katerem izmed ostalih ekvivalentnih modelov.%bolj točno program turing stroja in turing zato, ker je najbl cute
%-----------------------predavanja 5-------------------------------
\begin{comment}
%what is this I don't even... isto kot povzetki zgoraj, le za TS
\br
...
%@wtf problem s č-jem v naslovu
\chap{Zahtevnost racunanja}
...
\Def{Naj bo $f:\mathbb{N}\rightarrow\mathbb{N}$ funkcija, kjer velja:\\
	$sup(f(n))_{n\rightarrow\infty} \PoDef lim_{n\rightarrow\infty} (LUB\{ f(n), f(n+1), \dots \})$ %lowest upper bound e
	$inf(f(n))_{n\rightarrow\infty} \PoDef lim_{n\rightarrow\infty} (GLB\{ f(n), f(n+1), \dots \})$ %greatest lower bound
}
\Primer{ 
...
\br
Torej ne obstaja tak Turingov stroj, ki bi sprejel jezik $L$ s časovno omejitvijo $T(n)$.
}
...
Torej obstaja neskončna hiearhija razredov prostorske zahtevnosti, ki je \Quote{precej gosta}}%"lol" ... also dokaz tega je težak, je pa v stari skripti
...
\Izrek{NSPACE($f(n)$) $\subseteq$ DSPACE($f(n)^2$), če $f(n) \geq \log_2 \wedge f(n)$ je popolnoma prostorsko predstavljiva.\\
Torej, kar lahko rešimo na nedeterminističen način na prostoru $O(f(n))$, lahko na deterministični način na prostoru največ $O(f(n)^2)$.}
\Dokaz{%@nimaš a),b... označeno
\odmakni{a)}{Trivialno.}
\odmakni{b)}{
...
Ampak predvidevamo, da velja le $\subset$(ne $\subseteq$), saj $g(n)$ narašča dovolj hitro, da bo gotovo $inf {S(n) \over g(S(n))} = 0$, kar zahteva tudi to, da je $g(S(n))$ prostorsko predstavljiva.
\odmakni{Toda}{
Izrek zahteva tudi, da je g(S(n)) prostorsko predstavljiva, mi pa tega nismo zahtevali v (neki)\\%??
To nam izniči pričakovanje da velja $\subset$ in ne $\subseteq$. Lahko se zgodi, da povečanje (celo bistveno) prostorske omejiteve iz $S(n)$ na $g(S(n))$.\\
...
\br
Ali $ \exists S(n) $ : DSPACE($S(n)$) vsebuje vse DSPACE($p(n)$)\\
...
\Odmakni{Uporaba}{
$f_i(n)=n^i$\\
Obstaja $S(n)$, da je DSPACE($f(n)$ = $\bigcup_{i \geq 1}$ DSPACE($n^i$)
}
Izrek o uniji velja tudi za DTIME, NSPACE in NTIME.
\Posledica{
$P \PoDef \bigcup_{i \geq 1}$ DTIME($n^i$)\\

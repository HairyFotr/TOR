...
Za $\delta$ sedaj velja: 
$\delta:Q\times\Gamma \rightarrow 2^{Q\times\Gamma\times\{ L,D,- \}}$\\
Torej ima pri nekem stanju in tračnem znaku tak TS več kot le eno trojko iz $Q\times\Gamma\times\{ L,D,- \}$. Toda teh trojk je le končno mnogo.

\Primer{
$\delta(0, a) = \{ \langle q_1, b, L \rangle, \langle q_2, c, D \rangle, \langle q_3, a, L \rangle, \langle q_2, B, L \rangle \}$
stroj bo izbral tisto preslikavo, ki ga vodi k sprejetju vhodne besede. Če to ni možno, se bo stroj ustavil ali pa bo mlel v neskončnost.
}
...
%
%
Naj bo M nek NTS. Naj ima njegov program v vsaki mmnožici $\delta(q,a)$ kvečjemu $r$ možnih potez. $r$ je končno število.\\
Stroju M priredimo osnovni 3-sledni TS M', ki:
\begin{items}
\item ima na prvi sledi abecedo stroja M
\item na drugi sledi generira zaporedje navodil besed nad abecedo ${1, 2, ..., r}$ v leksikografskem vrstnem redu (npr. $r = 3: 1,2,3,11,12,13,...)$
\item na tretji sledi simulira M, kot da bi M izbral svoje poteze skladno stekočimi navodili
\end{items}
Natančneje:
\begin{items}
\item na drugi sledi sestavi novo, naslednje navodilo
\item prepiše vhodno besedo s prvega na tretjo sled
\item na tretji sledi oponaša stroj M, kot da bi ta izbral svoje poteze po tekočem navodilu.\\
      Pri tem:
      \begin{items}
      \item če M' pride do konca navodila in je tedaj v končnem stanju, vhodno besedo sprejme in konča, kot bi to storil tudi M
      \item sicer nadaljuje s prvim korakom ali ša se ustavi v končnem stanju
      \end{items}
\end{items}
}

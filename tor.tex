...
Vrste problemov:
\begin{items}
\item Odločitveni - odgovorijo z DA/NE %decision yes/no... ali ima graf Hamiltonov cikel, ali je št prašzevilo
\item Iskalni - sprašuje po odlikovanem elementu neke množice
\item Preštevalni - število ... v množici
\item Naštevalni - generiranje vseh odlikovanih elementov
\end{items}
Odločitevni so najenostavnejši, zato se jim bomo tu bolj posvetili.

\subsect{Jezik odločitvenih problemov}
Problemu priredimo nek formalen jezik - množica besed nad neko

Naj bo $P$ odločitveni problem, npr. $P$="Ali je $n$ praštevilo?".
V praksi namesto splošnega parametra $n$ vstavimo konkretni podatek, npr. "Ali je 19871 praštevilo". To je naloga.
Naj bo $p$ naloga problema $P$. Vsaka naloga ima odgovor DA/NE. Glede na to je naloga pozitivna/negativna.
...
Temu jeziku pripada odločitveni problem $P_\mathcal{K}$ - "ali se Turingov stroj ustavi nad lastno kodo?".

\Odmakni{Lema}{Problem $P_\mathcal{K}$ ni odločljiv.}
...
\fixme škatla N. vhod <T>,v škatli naredi <T,T>, preda kosmati (ne vemo če obstaja) škatli R_K z izhodi DA/NE... če DA ga vprašamo šeenkrat isto.. če ne, ne speljemo na izhod N
\Trditev{domnevni R_K napove prihodnost stroja T, N pa bo razgalil nesposobnost R_K pri tem napovedovanju na primeru T=N}
...
Domnevni R_K ne bi rešil naloge p - "ali se Turingov stroj N pri vhodu <N> ustavi?"
Sklep: K ni rekurziven jezik, zato R_K ni odločljiv problem.%problem, ki ni odločljiv!!
}
}%konec leme
...
\Odmakni{Problem o TS:}{
Problem garača(busy beaver).
Vzamemo Turingove stroje, ki imajo n nekončnih stanj. Takih strojev je končno mnogo. Garač je stroj z n nekončnimi stanji, ki začne s praznim trakom in zapiše največ enic preden se ustavi.
"Ali je Turingov stroj T garač?".

Še nekaj primerov za ustavitev TS: "ali se T pri w ustavi", "ali se T pri praznem vhodu ustavi?", "ali se T pri vsakem vhodu ustavi", "ali se dva stroja ustavita pri istih vhodih?"

Lastnosti jezikov TS: "Ali T razpoznava rekurziven jezik.", "ali T razpoznava regularen jezik?", ...

Problemi o algoritmih in programih:
P program... <P> je strog opis problema. A algoritem... <A> program
Pravilnost algoritma: "ali program <A> rešuje problem <P>"

Najkrajši program:
Pravimo, da je program ekvivalenten <A>, če je za vsak vhod enak kot <A>. 
"Ali obstaka krajši program, ki je ekvivalenten <A>"

Problemi o izračunljivih funkcijah:
$\phi:A\rightarrow B$ je izračunljiva. "Ali ima $\phi$ neprazno domeno?", "Ali ima $\phi$ neskončno domeno?", "Ali ima $\phi$ končno domeno?", "Ali je $\phi$ surjektivna?", "Ali je $\phi$ totalna?"

Problemi iz matematike:
Diofantske enačbe: "ali ima DE $p(x_1,x_2,\dots,x_n)$ celoštevilsko rešitev?" %glej na začetek poglavja

Ničelni matrični produkt: \mathcal{M}=\{ M_1, M_2, \dots, M_n \} do matrike reda $n\times n$ s celoštevilskimi koeficienti. "ali je mogoče zmnožiti $M_i$, da dobimo ničelno matriko?"

Problemi o gramatikah in jezikih:
%nahitro
Dvoumnost KNG. (Obstoj stavka, ki ima dve različi izpeljavi v KNG.)
Ekvivalenca KNG.%recimo zanimivo za načrtovalce jezikov.

Problemi matematične logike:
%tu je menda nek uvod, ki ga ne poznamo
"Ali so vse formule predikatnega računa prvega reda odločljive?"

%zdj pridejo luštni problemi ^^
...
Tlakovanje: Imamo končno množico tlakovcev $T=\{ \mbox{križ-kraž kvadrati z različno pobarvanimi četrtinami} \}$. Radi bi tlakovali ravnino razdeljeno na kvadrate, tako, da se vedno sosednja tlakovca ujemata v barvi.%slikica :P
"Ali lahko tlakujemo vsak poligon v $\mathbb{Z}^2$", "ali lahko tlakujemo pot v ravini od točke A do C, s tem, da ne obiščemo točke B"
}

\subsect{Dokazovanje neodločljivosti}
Metode dokazovanja:
\begin{items}
...
\begin{items}
\item Izberi nek problem P, ki ni odločljiv. %how do i?
\item Dokazati moramo da velja: če bi bil $Q$ odločljiv, bi bil tudi $P$ odločljiv.
\item Če prejšnji korak uspe lahko sklenemo da Q ni odločljiv. %lol, modus tollens
\end{items}
V koraku 2 z domnevnim Turingovim strojem $R_\mathcal{Q}$, je treba sestavitit Turingov stroj $R_\mathcal{P}$, ki se vedno ustavi.

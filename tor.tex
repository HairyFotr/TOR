...
$f:\mathcal{N}\rightarrow\mathcal{M}$%
\Odmakni{?}{Kako na splošno dokazati, da je neka funkcija res totalna?\\
V najslabšem primeru, bi bilo treba preizkusiti za vsak $x\inA$, ali je funkcija pri tem $x$ definirana ( pišemo: $f(x)\uparrow$ )\\
...
\item rekurzivnih funkcije je \Aleph0 (števno mnogo)
\item Vsako definira program TS, programov pa je števno mnogo.
\item ? ali je med vsemi funkcijami ($\mathcal{N}^\mathcal{N}$) možno najti tako, ki je izračunljiva, pa ni rekurzivna.
\item DA -- dokaz z diagonalizacijo:\\
\end{items}
\Trditev{Obstaja izračunljiva funkcija, ki ni rekurzivna.}
\Dokaz{ 
	\begin{items}
	\item definicije funkcij so končna zaporedja.%(Trditev)
	\item uredimo zaporedja po dolžini, enako dolga pa leksikografsko
	\item => lahko govorimo o prvem, drugem, ..., o $n$-tem programu TS. Zato lahko tudi prvi, drugi, ... n-ti rekurzivni funkciji.
	\item označimo n-to rekurzivno funkcijo z f_n 
	\item definirajmo ......
		g(n, a_1,\dots, a_k) po def. f_n(a_1, a_2, \dots, a_k}+1, kjr $a_i\in \mathcal{N}$%označimo z *

funkcija je izračunljiva
algoritem je ....
....
\item ali je  g tudi rekurzivna
	ali obstaja program TS, ki je izračuna.
	
\item predpostavka: g je rekrivna
	Tedaj obstaja nek naravni m, da je f_m = g.%nti program po vrsti. označimo z **
	Poglejmo vrednost g(mmmm)
	...
	kaj sedaj? 
	Ali naj dodajo začetne funkcije, ali pravila za dodajanje, če gledamo model ... Godel
...
%1996 je en rus sestavil nekaj različic stroja U... 
% |Q_u|  |\Sigmau|
%   15    2
%   9     3
%   6     4  
...
%   3     9
%   2    18
%kaj je wolfram tuki s cellularnimi počel?
%eniac, ....
\sect{Pravi stroji, ki so univerzalni}
Spremembe:

...
Na $\langle T \rangle$ lahko gledamo kot da je dvojiški zapis nekega naravnega števila in indeks Turingovega stroja $T$.
\br
Nekatera naravna števila niso indeksi Turingovega stroja, zato se dogovorimo: če naravno število nima oblike $\langle T \rangle$, rečemo, da je indeks praznega Turingovega stroja -- njegova $\delta$ je povsod nedefnirana, torej se takoj ustavi in ne sprejme nobene besede.
...
\subsubsect{Ideja za univerzalni Turingov stroj}
\Odmakni{Trakovi}{
\textbf{Vhodni trak} -- vsebuje vhodno besedo sestavljeno iz dveh delov:
\begin{items}
\item Kodo $\langle T \rangle$ poljubnega Turingovega stroja.
\item Poljubno besedo $w \in \Sigma^*$
\end{items}
\textbf{Delovni trak} -- sprva prazen. Stroj U .......\\
\textbf{Pomožni trak} -- sprva prazen. Stroj U ga bo uporabljal za zapis tekočega stanja stroja T in za primerjanje tega stanja s končnim stanjem stroja T.
}
\ \\
...
\begin{tikzpicture}[vert/.style={circle, draw,fill=black, inner sep=1pt}]
	...
	\node (s) at (6.2cm, 2.5cm) {};
	\node [below=0.5pt] at (s.south) {$\Sigma^*$};

	\draw [decorate,decoration={random steps,segment length=2pt,amplitude=1pt}] 

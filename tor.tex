...
\item V kakšnem pomnilniku so ukazi shranjeni?
	...
	\end{items}
\end{items}
Nekateri zgodnji poskusi formalizacije pojma algoritma so: \fixme%? zgodovinsko zaporedje, letnice, imena modelov, moar info
\begin{items}
...
\section{Moč TS}
V moč Turingovega stroja se prepričamo tako, da dokažemo, da so navidez močnejši stroji ravno tako močni kot osnovni Turingov stroj. Ogledali pa si bomo tudi nekaj drugačnih načinov računanja.

Vsi so enako močni

Postopek dokazovanja: Če je $\mathcal{M}$ razred modelov, za katerega želimo dokazati, da so ekvivalentni TS, moramo poiskati sistematičen, končen postopek S, ki poljubnemu stroju $M \in \mathcal{M}$ priredi nek TS $M'$, ki je sposoben simulirati $M$.

\fixme - slika krog $\mathcal{M}$ z točko M -> TS z točko M

\subsubsect{Turingov stroj z dvosmernim trakom}
Trak je neomejen v obe smeri
Vhodna beseda se vpiše nekam na trak, okno se prestavi na prvi znak besede

\ \\
\begin{center}
\begin{tikzpicture}[start chain, node distance=-0.075mm, thick, cell/.style={draw, very thin, on chain, minimum size=0.6cm}]
	\node ({end}) [cell, thick, draw=white] {};
	\node ({Ad2}) [cell] {\ ...\ \ };
	\node ({Ai}) [cell, ultra thick] {};
	\node ({A1}) [cell, minimum width=4cm] {$w$\ \ \ \ \ };
	\node ({Ab1}) [cell] {$B$};
	\node ({Ab1}) [cell] {$B$};
	\node ({Ad2}) [cell] {\ ...\ \ };
	\node ({end}) [cell, thick, draw=white] {};
...
Stroj je definiran enako: $M=\langle Q,\Sigma,\Gamma,\delta,q_0,B,F \rangle$
le $\delta$ je enostavnejša, saj ni treba skrbeti kaj se zgodi, ko zadanemo levi rob.

Trditev: ni šibkejši od osnovnega
Dokaz: lahko se vede kot da je omejen na levi

?: Ali je močnejši od osnovnega? Ne.
...
\end{tikzpicture}
\end{center}
Priredimo mu dvosledni TS M'
\ \\
\begin{center}
\begin{tikzpicture}[start chain, node distance=-0.075mm, thick, cell1/.style={draw, very thin, on chain=1, minimum size=0.6cm}, cell2/.style={draw, very thin, on chain=2, minimum size=0.6cm}]
	\node ({end}) [cell1, thick, draw=white] {};
	\node ({Ad2}) [cell1] {\ ...\ \ };
	\node ({Ai}) [cell1, ultra thick] {};
	\node ({A1}) [cell1] {$A_{0}$};
	\node ({A1}) [cell1] {$A_{1}$};
	\node ({A1}) [cell1] {$A_{2}$};
	\node ({Ad2}) [cell1] {\ ...\ \ };
	...
	\node ({Ai}) [cell2, ultra thick] {};
	\node ({A1}) [cell2] {$A_{#}$};
	\node ({A1}) [cell2] {$A_{-1}$};
	\node ({A1}) [cell2] {$A_{-2}$};
	\node ({Ad2}) [cell2] {\ ...\ \ };
	\node ({end}) [cell2, thick, draw=white] {};
...
# bo povedal, da je okno na robu in je treba gledati na drugo sled.

M deluje tko, da 
upošteva (bere in piše) naeenkrat le eno sled in se obrne, ko vidi # na spodni sledi
na zgornji sledi dela enako
na spodnji pa obnemo levo/desno
(napisali še 2x isto?)
...
TS ima več trakov
... po eno na trak
na prvem traku je okno na prvem znaku besede

\fixme - slika več trakov

korak stroja $\delta = Q \times \Gamma^k \rightarrow Q\times(\Gamma\times\{ L,D,N \})^k$

odvisno od stanja in znakov na k-tih oknih naredi:
spremeni stanje 
neodvisno od drugih zamenja znake v oknih
premakne okna

Večtračni je enako močen
-> damo vhodno besedo in delamo vse na enem traku
<- večtračni TS M in 2k-sledni {v obe strani neskončni} osnovni M'
...
\subsubsect{Lamba račun}
Church

\subsubsect{Algoritmi Markova}
Markov

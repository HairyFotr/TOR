...
\begin{tikzpicture}[scale=1.35, very thick, circ/.style={thin, decorate, decoration={random steps, segment length=3pt, amplitude=0.75pt}}]
...
\fixme - slika večslednega stroja
...
\Odmakni{Postopek dokazovanja}{Recimo, da je $\mathcal{M}$ razred modelov, za katerega želimo dokazati, da so ekvivalentni TS, moramo poiskati sistematičen, končen postopek S, ki poljubnemu stroju $M \in \mathcal{M}$ priredi nek TS $M'$, ki je sposoben simulirati $M$.
\begin{center}
...
\Dokaz{Lahko se vede kot da je omejen na levi.\fixme - to smo delali na vajah}
\ \\
...
\subsubsect{Večtračni Turingov stroj}
...
\begin{tikzturing}[cell11/.style={cell1,fill=white}]
	\node [cell11] {\ ...\ \ };
	\node at (0bp,-1.6cm) [cell2]  {\ ...\ \ };

	\node (Ai) [cell11, selected] {$w_{0}$};
	\node (Ai2) [cell2, selected] {$B$};
...
\Def{Korak stroja $\delta$ opišemo kot:
	...
	Torej, na vsakem koraku dobimo iz trenutnega stanje glave, ter tračnega simbola iz vsakega traku, neko novo stanje, ter za vsak trak neodvisno nov simbol in premik.}

\Trditev{Večtračni Turingov stroj je enako močen kot osnovni model.}
\Dokaz{
	\Odmakni{($\Rightarrow$)}{Preprosto delamo vse le na prvem traku}
	\Odmakni{($\Leftarrow$)}{Vzamemo $k$-tračni Turingov stroj M in $2k$-sledni v obe strani neskončni M'.}}
\begin{comment}
		\fixme
		dve sledi za vsak trak.. na prvi oznka X, ki pove ...
		na spodnji je ... stroja M
	
	\fixme - slika z dvemi trakovi in en večsledni
%?------------------------neurejeno-----------------------------
...
stanje
prostor za k ... znakov
števec je omejen za štetje 0..k

M' simuliramo en korak stroja M tako
...pod njim in zmanjša števec za 1
ko števec pade na nič, se začne pomikati v levo
pri tem pomikanju v levo, vsakokrat, ko najde X  .. 
	pod njim  .... (kot bi zamenjal M v svojem ...)
	
	poveča števec za 1
ko števec naraste na k, se pomikanje v levo konča, nadzorna enota predie v novo stanje

Torej en korak stroja M simulira M' s končno dolgim sprehodom iz desne pa v levo?

%Formalizacija: nismo delal

enako sta močna

Izrek: za L obstaja večtrani TS ntk, obstaja za L običajni TS.

\subsubsect{Nedeterministični Turingov stroj}
$\delta:Q\times\Gamma \rightarrow 2^{Q\times\Gamma\times\{ L,D,N \}}$
...
\Def{NTS sprejme besedo, kadar obstaja konno zaporedje kononononooooooooooo!!! korakov, ki se konča v končnem stanju
}
NTS zmore vse kar zmore osnovni TS
Dokaz: $\delta$ osnovnega TS je le poseben primer delte NTS
NTS Ni močnejši
...
\subsubsect{Večdimenzionalni Turingov stroj}
... k>=2
...
\subsubsect{Rekurzivne funkcije}
%(GK) Gödel, Kleene
...
\Def{Totalne š.t. funkcije f:N^k:N, ki se dajo konstruirati iz treh začetnih funkcij s končno mnogo uporabami treh pravil sestavljanja se imenujejo rekurzivne funkcije.}
...
1. Če so dane funkcije g:N^m->N in n_h:N^k->N, kjer i 1-m, potem je funkcija f(n_1...n_k) po def g(h1(n...nk)m...,h_m(n...nk)) sestavljana s kompozicijo funckij
2. Le sta dani funkciji g:N^k->N in h:N^{k+1}->n, potem je f definiriana z \[f(n_1, ..., nk, 0) po def g(n_1, ..., n_k)\]
\[f(n_1, ..., nk, m+1) po def h(n_1, ..., n_k, m, f(n_1, ... n_k, m)) za m\geq 0\]
sestavljeno s .... in g in h
3.Če je funkcija g:N^k+1->N taka, da za vsako k-terico naravnih števil n_1, \dots, n_k obstaja n .. m, da je g(n_1, \dots, n_k, m) = 0, potem je funkcija:
\[ f(n_1, \dots, n_k) po def \mu x g(n_1, \dots, n_k, X)\]
...
%zaeenkrat nismo našli Ackermann2 :)
...
f je neznana funkcija, $g_1, \dots g_m$ pa znane %mestnost?
...
Kakšna so pravila za računanje vrednosti $f(n_1, \dots, n_k)$ iz $\varepsilon(f)$. %leta 1934 je ugotovil, da
Pravili sta samo 2.
...
\Odmakni{Povzetek}{Algoritem po Herbrant-Gödlu(HG) je $\varepsilon(f)$.\\
	Računanje po HG je izračun vrednosti funkcije f(n_1,\dots,n_k) iz $\varepsilon(f)$ z uporabo pravil 1,2 .\\%izr. je, sta rekla... oz je samo Gödel reku... :D
	Funkcija je izračunljiva po HG, če je splošno rekurzivna.}

\subsubsect{$\lambda$ račun}%1932,1933 Alonso Church
...
\subsubsect{Postov Stroj}%1936, Post
%podoben turingovemu
%nadzorna enota
%trak s celicami
%okno
...
\subsubsect{Algoritmi Markova}%194..
Imamo abecedo $\Sigma$, kočno zaporedje produkcij:
$x_1 \leftarrow y_1$
$x_2 \leftarrow y_2$
...
$x_n \leftarrow y_n$
...
\subsection{Church-Turingova teza}

...
\item $\sqrt[3]{2}$ je racionalno število.
	\begin{items}
	\item Če je $\sqrt[3]{2}$ racionalno število, ga je moč zapisati kot ulomek.
	\item Predpostavimo, da je ulomek okrajšan, torej, da velja: $GCD(a,b)=1$):
		\begin{align*}
		\sqrt[3]{2} &= \frac{a}{b}\\
...
\begin{tikzpicture}[>=latex',/tikz/initial text=""]
	...
	\node (q1) at (100bp,0bp) [state, accepting] {$q_e$};

	\draw [->] (q0) to[loop above] node[auto] {$R$} (q0);
	\draw [->] (q0) to[bend left]  node[auto] {$S$} (q1);
	\draw [->] (q1) to[bend left]  node[auto] {$T$} (q0);
	\draw [->] (q1) to[loop above] node[auto] {$Q$} (q1);
\end{tikzpicture}
\\
...
\begin{tikzpicture}[>=latex',/tikz/initial text=""]
  ...
  \draw [->] (q0) to[bend left]  node[auto] {$1$} (q1);
  \draw [->] (q1) to[bend left]  node[auto] {$1$} (q0);
  \draw [->] (q1) to[bend left]  node[auto] {$0$} (q2);
  \draw [->] (q2) to[bend left]  node[auto] {$0$} (q1);
...
\item $L_1 \circ L_2$ - stik 
\item $L_1^*$ - iteracija %? v zapiskih imam poudarjeno "po definiciji".. wat?
\item $L_1 \cap L_2$ - presek
\item $\overline L$ - komplement
